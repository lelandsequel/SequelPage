...
function buildPrompt(config: AnalysisRequest): string {
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!requestData.geo || !requestData.mode) {
      return new Response(
        JSON.stringify({ error: 'Missing required fields: geo and mode' }),
        { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }

    const weights = requestData.weights || DEFAULT_WEIGHTS;

    const { data: analysisRecord, error: insertError } = await supabase
      .from('industry_analyses')
      .insert({
        geo: requestData.geo,
        timeframe: requestData.timeframe || 'last 6-12 months',
    ...
    if (insertError) {
      ...
      return new Response(
        JSON.stringify({ error: 'Failed to create analysis record', details: insertError.message }),
    ...
    const anthropicResponse = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        ...
        max_tokens: 8192,
        system: 'You are a senior SEO/AEO strategist and market analyst. You rigorously justify claims, separate assumptions from verified facts, and always return results in valid JSON.',
        messages: [
          {
            role: 'user',
            content: prompt,
    ...
    if (!anthropicResponse.ok) {
      ...
      await supabase
        ...
        .update({
          status: 'failed',
          error_message: `API error: ${anthropicResponse.status}`,
        })
        .eq('id', analysisRecord.id);

      return new Response(
        JSON.stringify({ error: 'Failed to analyze industries' }),
        { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }
    ...
    await supabase
      .from('industry_analyses')
      .update({
        ranked_industries: parsedResult.ranked_industries || [],
        methodology: parsedResult.methodology || {},
        status: 'completed',
